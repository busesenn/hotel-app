import { View, Text, ScrollView, Image, TouchableOpacity, Modal, Alert, Button } from 'react-native'
...
export default function PopularHotelsDetail() {
    ...
    const selectedHotel = Opportunities.find((hotel) => hotel.id === id);
    ...
    const saveReservation = async (reservation) => {
        try {
            const existingData = await AsyncStorage.getItem("reservations")
            ...
            console.error('Kayıt hatası:', error);
            Alert.alert("Rezervasyon kayıt edilirken bir hata oldu")
        }
    }
    ...
    return (
        <ScrollView style={{ flex: 1, backgroundColor: Color.navyBlue }}>
            ...
            <View style={{ marginLeft: 20, marginTop: 30, borderWidth: 1, borderColor: "gray", padding: 10, width: "90%", gap: 5, flexDirection: "row", borderRadius: 10 }}>
